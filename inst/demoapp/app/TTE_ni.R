...
## @param hma                   higher means are 1=worse 0=better
## @param nim:                  non-inferiority margin (always positive)
                      ...
                      ms0, hr, drop_ctrl, drop_trt, n_targetevent)
{
    ...
    ci <- c(cox_ph_main$conf.int[3], cox_ph_main$conf.int[4],
            cox_ph_cn$conf.int[3], cox_ph_cn$conf.int[4]) #confidence interval
    ...
    statistics <- c(coef, expcoef, ci, p_main, p_cn, n_event, target_eventdate, median_dur)
    names(statistics) <- c("logHR_main", "logHR_cn", "HR_main", "HR_cn", "CL_main",
                           "CU_main", "CL_cn", "CU_cn", "p_main", "p_cn", "n_event_main",
                           "n_event_cn", "targeteventdate",
...
ConsistecncyP_CE <- function(pattern, n, ratio, ratio_cn, pct, pct_in,
                             ...
                             n_targetevent, nsim, SigL, thd, hma, nim, seed)
{
    set.seed(seed)
    ...
    c(n_global = n
      ...
      , conditional_p = if(hma == 1){round(sum((final[, "CU_main"] < nim) &
                                                   (final[, "logHR_cn"] - log(nim))/(final[, "logHR_main"]-log(nim)) > thd)/
                                               sum(final[, "CU_main"] < nim), 3)}
      else{round(sum((final[, "CL_main"] > nim) &
                         (final[, "logHR_cn"] - log(nim))/(final[, "logHR_main"]-log(nim)) > thd)/
                     sum(final[, "CL_main"] > nim), 3)}
      , unconditional_p = round(sum((final[, "logHR_cn"] - log(nim))/
                                        (final[, "logHR_main"]-log(nim)) > thd)/nsim, 3)
      , cond_p_red = if(hma == 1){round(sum((final[, "CU_main"] < nim) &
                                                ((1-final[, "HR_cn"]/nim)/(1-final[, "HR_main"]/nim) > thd))/
                                            sum(final[, "CU_main"] < nim), 3)}
      else{round(sum((final[, "CL_main"] > nim) &
                         ((1-final[, "HR_cn"]/nim)/(1-final[, "HR_main"]/nim) > thd))/
                     sum(final[, "CL_main"] > nim), 3)}
      , uncond_p_red = round(sum((1 - final[, "HR_cn"]/nim)/(1 - final[, "HR_main"]/nim) > thd)/nsim, 3)
      ...
      , power_global = if(hma == 1){round(sum(final[, "CU_main"] < nim)/nsim, 3)}
      else{round(sum(final[, "CL_main"] > nim)/nsim, 3)}
      , power_cn = if(hma == 1){round(sum(final[, "CU_cn"] < nim)/nsim, 3)}
      else{round(sum(final[, "CL_cn"] > nim)/nsim, 3)}
      , threshold = thd
    )
}
...
plotSim <- function(pattern = 1, chinese_all_in = 1 , pct_seq = NULL,
                    ...
                    n_targetevent = 227, nsim = 10, SigL = 0.05, thd = 0.5, hma=1, nim=1.2, seed = 12345
) {
    # snowfall support
...
#          n_targetevent = 333, nsim = 100, SigL = 0.05, thd = 0.5, hma=1, nim=1.2, seed = 12345)
