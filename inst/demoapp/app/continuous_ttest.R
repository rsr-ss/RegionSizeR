...
)
{
  ...
  single_simu<-function(nsim=1,ss=ss,mean_trt=mean_trt,mean_ctrl=mean_ctrl,
                        sd_ctrl= sd_ctrl,sd_trt=sd_trt,SigL=SigL,altHypo=altHypo,thd=thd)
  {
    ...
    ratio = diff(cn$estimate) / diff(all$estimate)

    # conditions
    cond = ratio > thd & all$p.value < SigL
    uncond = ratio > thd
    names(ratio)<-NULL
    names(cond)<-NULL
    names(uncond)<-NULL
  ...
  rr<-replicate(nsim,single_simu(ss=ss,mean_trt=mean_trt,mean_ctrl=mean_ctrl,
                                 sd_ctrl= sd_ctrl, sd_trt=sd_trt,SigL=SigL,altHypo=altHypo,thd=thd),simplify=FALSE)
...
ConsistecncyP_CE <- function(n, ratio=1, ratio_cn=1,pct = 0.2, pct_in = 1,
  ...
  power_global=sum(res$pvalue < SigL)/ nsim
  power_cn=sum(res$pvalue_cn < SigL)/ nsim
  conp = round(sum(res$cond==TRUE) / sum(res$pvalue < SigL),2)
...
#' @param pct Number of Chinese subjects in global study/Number of Chinese subjects(%)
#'            Note only show as parameter if "partially in" for Chinese_all_in selected
...
plotSim <- function(chinese_all_in = NULL,
  ...
  if(chinese_all_in == "yes" || chinese_all_in == "no") {
    ...
    if (nsim<=1000){
      ...
      print(system.time(result_df <- map_dfr(pct_seq, ~ConsistecncyP_CE(n=n, ratio=ratio,ratio_cn=ratio_cn,
                                                pct = .x,
                                                pct_in= pct_in,
                                                mean_ctrl=mean_ctrl,
                                                mean_trt=mean_trt,
    ...
    process <- function(parallel=FALSE,cpus=NULL){
      ...
      inner_ce<-partial(ConsistecncyP_CE,n=n, ratio=ratio, ratio_cn=ratio_cn,pct_in= pct_in,
                        ...
                        nsim=nsim, thd=thd, SigL=SigL, altHypo=altHypo, seed=seed)

      print(system.time(res <- sfLapply(pct_seq,inner_ce)))
      sfStop()
    ...
    g = ggplot(result_df1, aes(x=pct)) +
      ...
      labs(x = "N of subjects in subpopulation/N of subjects in global study(%)",
   ...
   if(nsim<=1000){
    ...
    g = ggplot(result_df1, aes(x=pct_in)) +
     geom_point(aes(y=prob, color = consistency) ) +
      scale_y_continuous(limits = c(min(result_df1$prob),1)) +
      labs(x = "N of subpopulaiton within global study/N of subpopulation(%)",
           y = "Probability of consistency") +
      theme_minimal()
  }
